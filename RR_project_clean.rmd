...
labels = c("No default", "Default")) + labs( title = "Disbursal Date by
Loan Default Status", x = "DISBURSAL DATE", y = "Number of Loans", fill
= "Loan Default" ) + theme_minimal()
...
plot_distribution_comp <- function(features, nrow = 2) { plots <- list()

for (feature in features) { p <- ggplot(train, aes_string(x = feature,
color = "factor(LOAN_DEFAULT)")) + geom_density(na.rm = TRUE, size =
1.2) + labs(title = paste("Distribution of", feature), x = feature, y =
"Density", color = "LOAN_DEFAULT") + theme_minimal() +
scale_color_manual(values = c("0" = "deepskyblue", "1" = "deeppink"),
labels = c("0 (No Default)", "1 (Default)")) plots[[length(plots) + 1]] <- p }
...
obs \<- length(train\$DISBURSED_AMOUNT) cat("No. of observations in
column:", obs, "\n")

# calculate summary statistics

data_mean \<-
mean(train$DISBURSED_AMOUNT, na.rm = TRUE) data_sd <- sd(train$DISBURSED_AMOUNT,
na.rm = TRUE) cat(sprintf("Statistics: Mean = %.3f, Std dev = %.3f\n",
data_mean, data_sd))

# identify outliers

cut_off \<- data_sd \* 3 lower \<- data_mean - cut_off upper \<-
data_mean + cut_off

outliers \<- train$DISBURSED_AMOUNT[train$DISBURSED_AMOUNT \< lower \|
train\$DISBURSED_AMOUNT \> upper] cat("Identified outliers:",
length(outliers), "\n")

train$DISBURSED_AMOUNT_new <- sapply(train$DISBURSED_AMOUNT, function(x)
{ if (is.na(x)) { return(NA) } else if (x \<= lower \|\| x \>= upper) {
return(data_mean) } else { return(x) } })

# No. of observations

cat("No. of observations in column:",
length(train\$DISBURSED_AMOUNT_new), "\n")

bin_labels \<- c("Low", "Medium", "High", "Extreme")

quantiles \<- quantile(train\$DISBURSED_AMOUNT, probs = c(0, 0.25, 0.5,
0.75, 1), na.rm = TRUE)

# Bin labels

train$DISBURSED_AMOUNT_bins <- cut(train$DISBURSED_AMOUNT, breaks =
quantiles, include.lowest = TRUE, labels = bin_labels)

# No. of observations in each category

table(train\$DISBURSED_AMOUNT_bins)

plot_bar("DISBURSED_AMOUNT_bins")

summary(train\$ASSET_COST) plot_distribution \<- function(feature, color
= "steelblue") { ggplot(train, aes_string(x = feature)) +
geom_histogram(aes(y = ..density..), fill = color, bins = 100, alpha =
0.7, na.rm = TRUE) + geom_density(color = "black", size = 1, na.rm =
TRUE) + labs(title = paste("Distribution of", feature), x = feature, y =
"Density") + theme_minimal() } plot_distribution("ASSET_COST", "tomato")
plot_box("ASSET_COST", "tomato")

#Number of observations in column

outlier_data \<- function(df, feature) { #Number of observations in
column obs \<- length(df[[feature]]) cat("No. of observations in
column:", obs, "\n")

\# calculate summary statistics data_mean \<- mean(df[[feature]], na.rm
= TRUE) data_sd \<- sd(df[[feature]], na.rm = TRUE)
cat(sprintf("Statistics: Mean = %.3f, Std dev = %.3f\n", data_mean,
data_sd))

\# identify outliers cut_off \<- data_sd \* 3 lower \<- data_mean -
cut_off upper \<- data_mean + cut_off

outliers \<- df[[feature]][df[[feature]] \< lower \| df[[feature]] \>
upper] cat("Identified outliers:", length(outliers), "\n")

return(list(lower = lower, upper = upper, mean = data_mean)) }

impute_outlier \<- function(vec, lower, upper, mean_val) { sapply(vec,
function(x) { if (is.na(x)) { return(NA) } else if (x \<= lower \|\| x
\>= upper) { return(mean_val) } else { return(x) } }) }
outlier_data(train,"ASSET_COST")

bin_labels \<- c("Low", "Medium", "High", "Extreme")

stats_asset \<- outlier_data(train, "ASSET_COST")

train$ASSET_COST_new <- impute_outlier(train$ASSET_COST,
stats_asset$lower, stats_asset$upper, stats_asset\$mean)
...
quantiles \<- quantile(train\$ASSET_COST, probs = c(0, 0.25, 0.5, 0.75,
1), na.rm = TRUE)

train$ASSET_COST_bins <- cut(train$ASSET_COST, breaks = quantiles,
...
summary(train\$LTV) plot_distribution("LTV", "blue") plot_box("LTV",
"blue") outlier_data(train,"LTV")

ltv_stats \<- outlier_data(train, "LTV")

train$LTV_new <- impute_outlier(train$LTV,
ltv_stats$lower, ltv_stats$upper, ltv_stats\$mean)
...
ltv_quantiles \<- quantile(train\$LTV, probs = c(0, 0.25, 0.5, 0.75, 1),
na.rm = TRUE)

train$LTV_bins <- cut(train$LTV, breaks = ltv_quantiles, include.lowest
= TRUE, labels = bin_labels)

table(train$LTV_bins) plot_bar("LTV_bins") summary(train$PERFORM_CNS_SCORE)
...
train$PERFORM_CNS_SCORE_bins <- cut(train$PERFORM_CNS_SCORE, breaks =
cut_bins, labels = bin_labels, include.lowest = TRUE, right = TRUE)

table(train\$PERFORM_CNS_SCORE_bins)

plot_bar("PERFORM_CNS_SCORE_bins")

train %\>% group_by(PERFORM_CNS_SCORE_DESCRIPTION,
PERFORM_CNS_SCORE_bins) %\>% summarise(count = n(), .groups = "drop")
%\>% arrange(desc(count))

table(train\$PERFORM_CNS_SCORE_DESCRIPTION)

gg \<- train %\>% group_by(PERFORM_CNS_SCORE_DESCRIPTION, LOAN_DEFAULT)
%\>% summarise(counts = n(), .groups = "drop") %\>%
group_by(PERFORM_CNS_SCORE_DESCRIPTION) %\>% mutate(percentage = counts
/ sum(counts) \* 100) %\>% ungroup()
...
stats_pri$lower, stats_pri$upper, stats_pri\$mean)

outlier_data(train, "PRI_NO_OF_ACCTS_new")

bin_labels \<- c("One", "More than One") cut_bins \<- c(-1, 1, 1000)

train$PRI_NO_OF_ACCTS_bins <- cut(train$PRI_NO_OF_ACCTS, breaks =
cut_bins, labels = bin_labels, include.lowest = TRUE, right = TRUE)
...
summary(train\$PRI_OVERDUE_ACCTS) plot_box("PRI_OVERDUE_ACCTS", "blue")
...
stats_overdue$lower, stats_overdue$upper, stats_overdue\$mean)

outlier_data(train, "PRI_OVERDUE_ACCTS_new")

bin_labels \<- c("None", "One (or more)") cut_bins \<- c(-1, 0, 1000)

train$PRI_OVERDUE_ACCTS_bins <- cut(train$PRI_OVERDUE_ACCTS, breaks =
cut_bins, labels = bin_labels, include.lowest = TRUE, right = TRUE)
...
plot_bar_comp \<- function(features, nrow = 2) { plots \<- list()

for (feature in features) { p \<- ggplot(train, aes_string(x =
"factor(LOAN_DEFAULT)", fill = "factor(LOAN_DEFAULT)")) + geom_bar(data
= train, aes_string(y = "..count.."), position = "dodge", color =
"black") + facet_wrap(as.formula(paste("\~", feature))) +
scale_fill_manual(values = c("0" = "skyblue", "1" = "tomato"), name =
"Loan Default", labels = c("No Default", "Default")) + labs(title =
paste("Distribution by", feature), x = "LOAN_DEFAULT", y = "Count") +
theme_minimal() plots[[length(plots) + 1]] \<- p }

do.call(grid.arrange, c(plots, nrow = nrow)) }

var \<- c("MOBILENO_AVL_FLAG", "AADHAR_FLAG", "PAN_FLAG",
"VOTERID_FLAG", "PASSPORT_FLAG", "DRIVING_FLAG") plot_bar_comp(var, nrow
= 3)
