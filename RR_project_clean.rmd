...
install.packages("caret")   # Run this once if caret is not installed
library(caret)              # Load caret to access trainControl() and other functions
...
```
```{r}
# Preprocess y_train outside the train() call
...
k_fold <- 10  # or however many you want
...
# Extract and print the best cross-validated accuracy
best_acc <- max(rf_cv_model$results$Accuracy)
cat("Cross-validated accuracy: ", round(best_acc * 100, 2), "%\n")
...
# Ensure y_train is a factor for classification
nb_model <- naiveBayes(x = X_train, y = factor(y_train))

# Predict on test set
# For naiveBayes, type="class" gives the predicted labels
...
# Create confusion matrix and print it
conf_matrix_nb <- confusionMatrix(nb_pred, y_test_factor)
cat("--- Naive Bayes Confusion Matrix ---\n")
print(conf_matrix_nb$table)

# Print accuracy as percentage
cat("\nAccuracy of model (Naive Bayes): ", round(conf_matrix_nb$overall["Accuracy"] * 100, 2), "%\n")

# Print other metrics
cat("F1 Score: ", conf_matrix_nb$byClass["F1"], "\n")
cat("Recall Score (Sensitivity): ", conf_matrix_nb$byClass["Sensitivity"], "\n")
cat("Balanced Accuracy Score: ", conf_matrix_nb$byClass["Balanced Accuracy"], "\n")

set.seed(101) # For reproducibility
nb_cv_model <- train(
  x = X_train,             # Training features
  y = factor(y_train),     # Target variable (as factor for classification)
  method = "naive_bayes",  # Method for Naive Bayes (using caret's interface)
  metric = "Accuracy",     # Primary evaluation metric
  trControl = ctrl         # Cross-validation control settings
)

NBCV <- nb_cv_model$results$Accuracy[which.max(nb_cv_model$results$Accuracy)] # Get best accuracy from CV results
cat("\nCross-validated accuracy (Naive Bayes): ", round(NBCV * 100, 2), "%\n")
```
##Stochastic Gradient Descent
```{r}
library(caret)
library(MLmetrics)  # For F1_Score, BalancedAccuracy
...
# Confusion Matrix
cm <- confusionMatrix(sgd_pred, y_test, positive = levels(y_test)[2])
print(cm$table)
...
# Balanced Accuracy
balanced_acc <- BalancedAccuracy(y_pred = sgd_pred, y_true = y_test)
cat("Balanced Accuracy Score:", round(balanced_acc, 3), "\n")

# Cross-validated accuracy (already done in caret train, but here’s how to extract)
...
dtree_model <- train(
  ...
  y = y_train,
  method = "rpart",
  tuneGrid = data.frame(cp = 0),  # cp = complexity parameter; we'll prune by maxdepth etc.
  trControl = ctrl,
...
# Confusion matrix
cm <- confusionMatrix(dtree_pred, y_test, positive = levels(y_test)[2])
print(cm$table)
...
# Balanced Accuracy
balanced_acc <- BalancedAccuracy(y_pred = dtree_pred, y_true = y_test)
cat("Balanced Accuracy Score:", round(balanced_acc, 3), "\n")
...
library(xgboost)
library(caret)
library(MLmetrics)  # for F1_Score and BalancedAccuracy
library(e1071)      # for confusionMatrix

set.seed(101)

# Make sure y_train and y_test are factors with consistent levels
if (is.list(y_train) || is.data.frame(y_train)) {
  y_train <- unlist(y_train)
}
y_train <- factor(y_train)
...
# Convert data to matrix for xgboost (caret does it internally but here for clarity)
# Usually X_train, X_test are already numeric matrices or data frames with numeric columns

# Train XGBoost model using caret with default parameters (you can tune as needed)
xgb_model <- train(
  x = X_train,
  y = y_train,
  method = "xgbTree",
...
# Confusion matrix
cm <- confusionMatrix(xgb_pred, y_test, positive = levels(y_test)[2])
print(cm$table)
...
# F1 Score
f1 <- F1_Score(y_pred = xgb_pred, y_true = y_test, positive = levels(y_test)[2])
cat("F1 Score:", round(f1, 3), "\n")
...
# Balanced Accuracy
balanced_acc <- BalancedAccuracy(y_pred = xgb_pred, y_true = y_test)
cat("Balanced Accuracy Score:", round(balanced_acc, 3), "\n")

# Cross-validated accuracy (best from caret training)
cv_accuracy <- max(xgb_model$results$Accuracy)
cat("Cross-validated accuracy:", round(cv_accuracy * 100, 2), "%\n")

```
## Dealing with Imbalanced DataAdd commentMore actions
